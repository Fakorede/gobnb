...
func (m *postgresRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	err := m.DB.QueryRowContext(
		ctx, stmt, res.FirstName, res.LastName, res.Email, res.Phone, res.StartDate, res.EndDate, res.RoomID, time.Now(), time.Now(),
	).Scan(&newID)
...
func (m *postgresRepo) InsertRoomRestriction(res models.RoomRestriction) error {
	...
	_, err := m.DB.ExecContext(
		ctx, stmt, res.StartDate, res.EndDate, res.RoomID, res.ReservationID, res.RestrictionID, time.Now(), time.Now(),
	)
...
// SearchAvailabilityByDatesByRoomID returns true if availability exists for roomID otherwise return false
...
// SearchAvailabilityForAllRooms returns all available rooms (if any) for given date range
